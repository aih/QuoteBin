...
from master.models import Movie, Tag, People, MoviePeople, MovieTag
    ...
    for e in x:
        ...
        try:
            ...
            try:
                ...
                movie = Movie.objects.create(imdb_rating=imdb_rating, votes = votes,
            ...
            info = m.getMovieInfo(id)
            for cat in info['categories'].values():
                    tags = cat.keys()
                    for t in tags:
                        obj, created = Tag.objects.get_or_create(name=t)
                        MovieTag.objects.get_or_create(movie=movie, tag=obj, user=None)
            for cast in info['cast'].values():
                for c in cast:
                    obj, created = People.objects.get_or_create(id=int(c['id']), defaults={'name':c['name']})
                    MoviePeople.objects.get_or_create(movie=movie, person=obj, role = c['job'], character_name=c['character'])
